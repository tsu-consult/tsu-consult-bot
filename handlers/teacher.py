...
async def show_teacher_tasks_page(callback: CallbackQuery, telegram_id: int, page: int):
    ...
    if not results:
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 К меню задач", callback_data="teacher_tasks_menu")]
        ])
        try:
            await callback.message.edit_text(text, reply_markup=keyboard)
        except TelegramBadRequest:
...
async def edit_task_reminders_start(callback: CallbackQuery, state: FSMContext):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⚙️ Настроить напоминания", callback_data="teacher_edit_reminder_custom")],
        [InlineKeyboardButton(text="🔕 Без напоминаний", callback_data="teacher_edit_reminder_none")],
        [InlineKeyboardButton(text="❌ Отмена", callback_data="teacher_cancel_edit_task")]
    ])
...
@router.callback_query(F.data == "teacher_edit_reminder_none")
...
@router.callback_query(F.data == "teacher_edit_reminder_custom")
async def teacher_edit_task_reminders_custom(callback: CallbackQuery, state: FSMContext):
    ...
    await teacher_show_edit_reminders_selection(callback, state)
    await callback.answer()


async def teacher_show_edit_reminders_selection(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    ...
    for minutes, label in reminder_options:
        ...
        keyboard_rows.append([
            InlineKeyboardButton(
                text=button_text,
                callback_data=f"teacher_edit_reminder_toggle_{minutes}"
            )
        ])

    if selected_reminders:
        keyboard_rows.append([
            InlineKeyboardButton(text="✅ Подтвердить выбор", callback_data="teacher_edit_reminder_confirm")
        ])

    keyboard_rows.append([
        InlineKeyboardButton(text="🔙 Назад", callback_data="teacher_edit_reminder_back")
    ])
...
@router.callback_query(F.data.regexp(r"^teacher_edit_reminder_toggle_(\d+)$"))
async def teacher_handle_edit_reminder_toggle(callback: CallbackQuery, state: FSMContext):
    telegram_id = callback.from_user.id
    role = await ensure_auth(telegram_id, callback)
    if role != "teacher":
        await callback.answer("Доступно только для преподавателей.", show_alert=True)
...
@router.callback_query(F.data == "teacher_edit_reminder_confirm")
async def teacher_handle_edit_reminder_confirm(callback: CallbackQuery, state: FSMContext):
    telegram_id = callback.from_user.id
    role = await ensure_auth(telegram_id, callback)
    if role != "teacher":
        await callback.answer("Доступно только для преподавателей.", show_alert=True)
...
@router.callback_query(F.data == "teacher_edit_reminder_back")
async def teacher_handle_edit_reminder_back(callback: CallbackQuery, state: FSMContext):
    telegram_id = callback.from_user.id
    role = await ensure_auth(telegram_id, callback)
    if role != "teacher":
        await callback.answer("Доступно только для преподавателей.", show_alert=True)
...
async def cancel_edit_task(callback: CallbackQuery, state: FSMContext):
    ...
    if task_id is not None and page is not None:
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 В главное меню", callback_data="teacher_tasks_menu")]
        ])
        try:
            await callback.message.edit_text(text, reply_markup=keyboard, parse_mode="HTML")
        except TelegramBadRequest:
...
@router.callback_query(F.data == "teacher_create_task")
async def teacher_start_create_task(callback: CallbackQuery, state: FSMContext):
    telegram_id = callback.from_user.id
    role = await ensure_auth(telegram_id, callback)
    if role != "teacher":
        await callback.answer("Доступно только для преподавателей.", show_alert=True)
...
async def teacher_handle_task_deadline_time(message: Message, state: FSMContext):
    ...
    try:
        time_dt = datetime.strptime(time_input, "%H:%M")
    except ValueError:
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="⚙️ Настроить напоминания", callback_data="teacher_task_reminders_custom")],
        [InlineKeyboardButton(text="🔕 Без напоминаний", callback_data="teacher_task_reminders_none")]
    ])
...
@router.callback_query(F.data == "teacher_task_reminders_default")
async def teacher_handle_reminders_default(callback: CallbackQuery, state: FSMContext):
    await state.update_data(reminders=None)
    await teacher_show_task_confirmation(callback, state)
    await callback.answer()


@router.callback_query(F.data == "teacher_task_reminders_none")
async def teacher_handle_reminders_none(callback: CallbackQuery, state: FSMContext):
...
async def teacher_show_reminders_selection(callback: CallbackQuery, state: FSMContext):
    ...
    for minutes, label in reminder_options:
        ...
        keyboard_rows.append([
            InlineKeyboardButton(
                text=button_text,
                callback_data=f"teacher_task_reminder_toggle_{minutes}"
            )
        ])

    if selected_reminders:
        keyboard_rows.append([
            InlineKeyboardButton(text="✅ Подтвердить выбор", callback_data="teacher_task_reminder_confirm")
        ])

    keyboard_rows.append([
        InlineKeyboardButton(text="🔙 Назад", callback_data="teacher_task_reminder_back")
    ])
...
async def teacher_show_task_confirmation(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Подтвердить", callback_data="teacher_confirm_create_task")],
...
@router.callback_query(F.data == "teacher_cancel_create_task")
async def teacher_cancel_create_task(callback: CallbackQuery, state: FSMContext):
    telegram_id = callback.from_user.id
    role = await ensure_auth(telegram_id, callback)
    await state.clear()
...
@router.callback_query(F.data == "teacher_confirm_create_task")
async def teacher_confirm_create_task(callback: CallbackQuery, state: FSMContext):
    telegram_id = callback.from_user.id
    role = await ensure_auth(telegram_id, callback)
    if role != "teacher":
        await callback.answer("Доступно только для преподавателей.", show_alert=True)
    ...
    if result:
        ...
        await callback.message.edit_text("✅ Задача успешно создана!", reply_markup=keyboard)
    else:
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 К меню задач", callback_data="teacher_tasks_menu")]
        ])
        await callback.message.edit_text("❌ Не удалось создать задачу. Попробуйте позже.", reply_markup=keyboard)
