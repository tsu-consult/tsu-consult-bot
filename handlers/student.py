...
async def start_create_request(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "Введите тему вашего запроса на консультацию 👇\n\n"
        "<b>⚠️ Внимание:</b> введённая тема будет видна всем пользователям. "
        "Если преподаватель создаст консультацию по вашему запросу, эта тема будет использована в качестве темы/названия консультации "
        "и впоследствии не подлежит изменению ни преподавателем, ни студентом.",
        parse_mode="HTML"
    )
    await callback.answer()
